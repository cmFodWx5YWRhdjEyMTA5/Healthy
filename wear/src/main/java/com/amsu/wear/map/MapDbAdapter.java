...
public class MapDbAdapter {
	...
	public List<PathRecord> queryRecordAll() {
		...
		while (allRecordCursor.moveToNext()) {
			...
			record.setId(allRecordCursor.getInt(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_ROWID)));
			record.setDistance(allRecordCursor.getString(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_DISTANCE)));
			record.setDuration(allRecordCursor.getString(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_DURATION)));
			record.setDate(allRecordCursor.getString(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_DATE)));
			String lines = allRecordCursor.getString(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_LINE));
			record.setPathline(MapUtil.parseLocations(lines));
			record.setStartpoint(MapUtil.parseLocation(allRecordCursor.getString(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_STRAT))));
			record.setEndpoint(MapUtil.parseLocation(allRecordCursor.getString(allRecordCursor.getColumnIndex(MapDbAdapter.KEY_END))));
	...
	public PathRecord queryRecordById(int mRecordItemId) {
		...
		if (cursor.moveToNext()) {
			record.setId(cursor.getInt(cursor.getColumnIndex(MapDbAdapter.KEY_ROWID)));
			record.setDistance(cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_DISTANCE)));
			record.setDuration(cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_DURATION)));
			record.setDate(cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_DATE)));
			String lines = cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_LINE));
			record.setPathline(MapUtil.parseLocations(lines));
			record.setAveragespeed(cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_SPEED)));
			record.setStartpoint(MapUtil.parseLocation(cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_STRAT))));
			record.setEndpoint(MapUtil.parseLocation(cursor.getString(cursor.getColumnIndex(MapDbAdapter.KEY_END))));
		}
		return record;
	}
