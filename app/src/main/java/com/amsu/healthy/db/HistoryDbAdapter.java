...
public class HistoryDbAdapter {
	...
	public List<RateRecord> queryRecordAll() {
		...
		while (allRecordCursor.moveToNext()) {
			...
			record.setId(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_ROWID)));
			record.setTime(allRecordCursor.getString(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_TIME)));
			record.setLfhf(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_LFHF)));
			record.setSdnn(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_SDNN)));
			String points = allRecordCursor.getString(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_RATELIST));
			record.setRatelist(parsePoints(points));
			record.setRatefilepath(allRecordCursor.getString(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_RATEFILEPATH)));
			record.setHrr(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_HRR)));
			record.setRatenormalcount(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_RATENORMALCOUNT)));
			record.setRatebadcount(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_RATEBADCOUNT)));
			record.setMissbeat(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_MISSBEAT)));
			record.setPrematurebeat(allRecordCursor.getInt(allRecordCursor.getColumnIndex(HistoryDbAdapter.KEY_PREMATUREBEAT)));
	...
	public RateRecord queryRecordById(int mRecordItemId) {
		...
		if (cursor.moveToNext()) {
			record.setId(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_ROWID)));
			record.setTime(cursor.getString(cursor.getColumnIndex(HistoryDbAdapter.KEY_TIME)));
			record.setLfhf(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_LFHF)));
			record.setSdnn(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_SDNN)));
			String points = cursor.getString(cursor.getColumnIndex(HistoryDbAdapter.KEY_RATELIST));
			record.setRatelist(parsePoints(points));
			record.setRatefilepath(cursor.getString(cursor.getColumnIndex(HistoryDbAdapter.KEY_RATEFILEPATH)));
			record.setHrr(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_HRR)));
			record.setRatenormalcount(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_RATENORMALCOUNT)));
			record.setRatebadcount(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_RATEBADCOUNT)));
			record.setMissbeat(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_MISSBEAT)));
			record.setPrematurebeat(cursor.getInt(cursor.getColumnIndex(HistoryDbAdapter.KEY_PREMATUREBEAT)));
		}
		return record;
	}
